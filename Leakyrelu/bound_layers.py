...
class BoundLinear(Linear):
    ...
    def bound_backward(self, last_uA, last_lA):
        def _bound_oneside(last_A, compute_A=True):
            ...
            if compute_A:
                next_A = last_A.matmul(self.weight) # 在这里乘了weight
        ...
        return uA, ubias, lA, lbias # 相当于计算了layer中每个neuron的关于该层输入对于最终输出的上下界参数?backward
...
class BoundReLU(ReLU):
    ...
    def bound_backward(self, last_uA, last_lA): 
        ...
        upper_d = ub_r / (ub_r - lb_r) # 这里的d就是论文里的D, 论文中是个对角矩阵, 这里大概是个向量
        upper_b = - lb_r * upper_d # upper_bias
        ...
        if self.bound_opts.get("same-slope", False):
            ...
            lower_d = (upper_d > 0.5).float() # 如果小于0.5就是直角三角形(lowerbound_slope=0), 否则lowerbound_slope=1
        uA = lA = None
        ubias = lbias = 0
        # Choose upper or lower bounds based on the sign of last_A
        # 我觉得这里可能需要配合weight食用 考察last_A是否乘过weight了(感觉在前传的过程中是不是确实...)
        if last_uA is not None:
            pos_uA = last_uA.clamp(min=0)
            if self.bound_opts.get("same-slope", False):
                # same upper_d and lower_d, no need to check the sign
                uA = upper_d * last_uA # 乘以d就相当于过了一个激活函数(point-wise)
            else:
                neg_uA = last_uA.clamp(max=0)
                uA = upper_d * pos_uA + lower_d * neg_uA # pos_uA和neg_uA里至少有一个是0
...
class BoundLeakyReLU(LeakyReLU):
    def __init__(self, prev_layer, inplace=False, bound_opts=None, negative_slope=0.01):
        super(BoundLeakyReLU, self).__init__(inplace)
        # ReLU needs the previous layer's bounds
        # self.prev_layer = prev_layer
        self.bound_opts = bound_opts
        self.negative_slope = negative_slope
    
    ## Convert a LeakyReLU layer to BoundLeakyReLU layer
    # @param act_layer ReLU layer object
    # @param prev_layer Pre-activation layer, used for get preactivation bounds
    @staticmethod
    def convert(act_layer, prev_layer, bound_opts=None, negative_slope=0.01):
        l = BoundLeakyReLU(prev_layer, act_layer.inplace, bound_opts, negative_slope)
        return l
    ...
    def interval_propagate(self, norm, h_U, h_L, eps):
        ...
        tightness_loss = self.unstab.sum()
        # tightness_loss = torch.min(h_U_unstab * h_U_unstab, h_L_unstab * h_L_unstab).sum()
        return norm, F.leaky_relu(h_U, negative_slope=self.negative_slope), F.leaky_relu(h_L, negative_slope=self.negative_slope), \
                tightness_loss, tightness_loss, (h_U < 0).sum(), (h_L > 0).sum()
    ...
    def bound_backward(self, last_uA, last_lA): 
        ...
        hq魔改
        '''
        lb_y = F.leaky_relu(lb_r, negative_slope=self.negative_slope)
        # CROWN upper and lower linear bounds
        upper_d = (ub_r - lb_y) / (ub_r - lb_r) # 这里的d就是论文里的D, 论文中是个对角矩阵, 这里大概是个向量
        upper_b = lb_y - lb_r * upper_d # upper_bias
        upper_d = upper_d.unsqueeze(1)
        if self.bound_opts.get("same-slope", False):
            # the same slope for upper and lower
            lower_d = upper_d
            ...
            # Always use slope 0 as lower bound. Any value between 0 and 1 is a valid lower bound for CROWN
            # lower_d = (upper_d >= 1.0).float()
            # lower_d = 1.0 if upper_d >= 1.0 else self.negative_slope
            lower_d = (upper_d >= 1.0).float() + self.negative_slope * (upper_d < 1.0)
        elif self.bound_opts.get("one-lb", False):
            # Always use slope 1 as lower bound
            # lower_d = (upper_d > 0.0).float()
            tmp = upper_d > self.negative_slope
            lower_d = tmp.float() + self.negative_slope * ~tmp
        else:
            # lower_d = (upper_d > 0.5).float() # 如果小于0.5就是直角三角形(lowerbound_slope=0), 否则lowerbound_slope=1
            # if upper_d >= 1.0 or upper_d <= self.negative_slope:
            #     lower_d = upper_d
            # else:
            #     lower_d = 1.0 if torch.abs(ub_r) > torch.abs(lb_r) else self.negative_slope
            tmp = (upper_d >= 1.0) | (upper_d <= self.negative_slope)
            tmp2 = (torch.abs(ub_r) > torch.abs(lb_r)).unsqueeze(1)
            lower_d = upper_d * tmp + ~tmp * (tmp2.float()+~tmp*self.negative_slope)
        '''
        魔改end
        '''

        # for up,lw in zip(upper_d.reshape(-1,1),lower_d.reshape(-1,1)):
        #   print(up,lw)
        ...
        ubias = lbias = 0
        # Choose upper or lower bounds based on the sign of last_A
        # 我觉得这里可能需要配合weight食用 考察last_A是否乘过weight了(感觉在后传的过程中是不是确实...)
        if last_uA is not None:
            pos_uA = last_uA.clamp(min=0)
            if self.bound_opts.get("same-slope", False):
                # same upper_d and lower_d, no need to check the sign
...
class BoundSequential(Sequential):
    ...
    def convert(sequential_model, bound_opts=None):
        ...
        for l in seq_model:
            ...
            if isinstance(l, LeakyReLU):
                layers.append(BoundLeakyReLU.convert(l, layers[-1], bound_opts, l.negative_slope))
    ...
    def full_backward_range(self, norm=np.inf, x_U=None, x_L=None, eps=None, C=None, upper=True, lower=True):
        ...
        for i in range(last_module + 1, len(modules)):
            ...
            # before a ReLU layer
            if isinstance(modules[i], BoundReLU) or isinstance(module, BoundLeakyReLU):
    ...
    def backward_range(self, norm=np.inf, x_U=None, x_L=None, eps=None, C=None, upper=False, lower=True, modules=None):
        ...
        for i, module in enumerate(reversed(modules)): # 这里back-propagation
        ...
        def _get_concrete_bound(A, sum_b, sign = -1):
            ...
            if norm == np.inf:
                ...
                bound = A.bmm(center) + sign * A.abs().bmm(diff) # IBP的计算方式
