...
def model_mlp_any(in_dim, neurons, out_dim = 10, negative_slope=0.01):
    ...
    # intermediate layers
    for n in neurons[1:]:
        units.append(nn.LeakyReLU(negative_slope=negative_slope))
...
def model_cnn_1layer(in_ch, in_dim, width, negative_slope=0.01): 
    model = nn.Sequential(
...
def model_cnn_2layer(in_ch, in_dim, width, linear_size=128, negative_slope=0.01): 
    model = nn.Sequential(
...
# parameter width: width multiplier
def model_cnn_3layer_fixed(in_ch, in_dim, kernel_size, width, linear_size = None, negative_slope=0.01):
...
def model_cnn_4layer(in_ch, in_dim, width, linear_size, negative_slope=0.01): 
    model = nn.Sequential(
...
def model_cnn_10layer(in_ch, in_dim, width, negative_slope=0.01): 
    model = nn.Sequential(
        # input 32*32*3
        nn.Conv2d(in_ch, 4*width, 3, stride=1, padding=1),
        nn.LeakyReLU(negative_slope=negative_slope),
